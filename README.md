...
- Consolidar o estudo em um material reutilizável (resumo, glossário e prompts)
...
Durante o estudo, eu testei prompts voltados para resolver problemas reais, como construir um sistema baseado em dados internos de uma empresa sem precisar treinar um modelo do zero.

### Prompt testado
> **Como fazer a criação de um LLM com apenas o contexto externo para os dados da minha empresa?**

### Resposta obtida (resumo)
A resposta trouxe a abordagem de **RAG (Retrieval-Augmented Generation)** como solução recomendada, explicando que o modelo não precisa ser treinado (fine-tuning), mas sim consultar documentos internos para responder perguntas com base no conteúdo recuperado.

Também foram citadas as etapas principais:

- Higienização e carregamento dos dados  
- Fragmentação (chunking)
- Vetorização e embeddings
- Armazenamento em banco vetorial
- Recuperação dos trechos relevantes
- Geração final com contexto
- Boas práticas de segurança (PII e controle de acesso)

---

### ⚠️ Dificuldades encontradas (Troubleshooting)

Alguns problemas apareceram durante os testes:

- **Respostas genéricas demais**  
  Muitas vezes o modelo respondia de forma “didática”, mas sem detalhar como aplicar em sistemas reais.

- **Falta de exemplos práticos**  
  Quando o prompt não pedia exemplos, a resposta vinha muito teórica.

- **Dificuldade em definir chunking ideal**  
  Ficou claro que chunking não é “tamanho fixo”: depende do tipo de documento e do objetivo da busca.

- **Risco de alucinação**
  Em perguntas mais abertas, o modelo poderia sugerir soluções não presentes no material. Isso reforçou a importância de sempre exigir referências e limitar a resposta ao conteúdo fornecido.
...
### 1) O que é um LLM
LLM (Large Language Model) é um modelo de IA treinado para compreender e gerar texto. Ele pode ser usado para automação, suporte técnico, geração de relatórios, análise de documentos e até interação com sistemas internos.
